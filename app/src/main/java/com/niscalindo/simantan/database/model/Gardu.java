package com.niscalindo.simantan.database.model;

import java.io.Serializable;

/**
 * Created by USER on 1/30/2018.
 */
public class Gardu implements Serializable {

    private String id;
    private String nomorGardu;
    private String alamat;
    private String kapasitasTrafo;
    private String merkTrafo;
    private String tapTrafo;
    private String jumlahJurusan;
    private String konstruksi;
    private String tanggalUkur;
    private String jamUkur;
    private String Petugas;
    private Double latitude;
    private Double longitude;
    private float zoom;
    private String penyulang;
    private String rn;
    private String rs;
    private String sn;
    private String rt;
    private String tn;
    private String st;
    private String ar;
    private String as;
    private String at;
    private String an;
    private String br;
    private String bs;
    private String bt;
    private String bn;
    private String cr;
    private String cs;
    private String ct;
    private String cn;
    private String dr;
    private String ds;
    private String dt;
    private String dn;
    private String fuseAr;
    private String FuseAs;
    private String FuseAt;
    private String fuseBr;
    private String FuseBs;
    private String FuseBt;
    private String fuseCr;
    private String FuseCs;
    private String FuseCt;
    private String fuseDr;
    private String FuseDs;
    private String FuseDt;
    private String noSeri;
    private String tanggalPasang;
    private String tanggalGanti;
    private String waktu;

    @Override
    public String toString() {
        return this.nomorGardu;
    }

    public String getNomorGardu() {
        return nomorGardu;
    }

    public void setNomorGardu(String nomorGardu) {
        this.nomorGardu = nomorGardu;
    }

    public String getAlamat() {
        return alamat;
    }

    public void setAlamat(String alamat) {
        this.alamat = alamat;
    }

    public String getKapasitasTrafo() {
        return kapasitasTrafo;
    }

    public void setKapasitasTrafo(String kapasitasTrafo) {
        this.kapasitasTrafo = kapasitasTrafo;
    }

    public String getMerkTrafo() {
        return merkTrafo;
    }

    public void setMerkTrafo(String merkTrafo) {
        this.merkTrafo = merkTrafo;
    }

    public String getTapTrafo() {
        return tapTrafo;
    }

    public void setTapTrafo(String tapTrafo) {
        this.tapTrafo = tapTrafo;
    }

    public String getJumlahJurusan() {
        return jumlahJurusan;
    }

    public void setJumlahJurusan(String jumlahJurusan) {
        this.jumlahJurusan = jumlahJurusan;
    }

    public String getKonstruksi() {
        return konstruksi;
    }

    public void setKonstruksi(String konstruksi) {
        this.konstruksi = konstruksi;
    }

    public String getTanggalUkur() {
        return tanggalUkur;
    }

    public void setTanggalUkur(String tanggalUkur) {
        this.tanggalUkur = tanggalUkur;
    }

    public String getJamUkur() {
        return jamUkur;
    }

    public void setJamUkur(String jamUkur) {
        this.jamUkur = jamUkur;
    }

    public String getPetugas() {
        return Petugas;
    }

    public void setPetugas(String petugas) {
        Petugas = petugas;
    }

    public String getPenyulang() {
        return penyulang;
    }

    public void setPenyulang(String penyulang) {
        this.penyulang = penyulang;
    }

    public Double getLatitude() {
        return latitude;
    }

    public void setLatitude(Double latitude) {
        this.latitude = latitude;
    }

    public Double getLongitude() {
        return longitude;
    }

    public void setLongitude(Double longitude) {
        this.longitude = longitude;
    }

    public float getZoom() {
        return zoom;
    }

    public void setZoom(float zoom) {
        this.zoom = zoom;
    }

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getRn() {
        return rn;
    }

    public void setRn(String rn) {
        this.rn = rn;
    }

    public String getRs() {
        return rs;
    }

    public void setRs(String rs) {
        this.rs = rs;
    }

    public String getSn() {
        return sn;
    }

    public void setSn(String sn) {
        this.sn = sn;
    }

    public String getRt() {
        return rt;
    }

    public void setRt(String rt) {
        this.rt = rt;
    }

    public String getTn() {
        return tn;
    }

    public void setTn(String tn) {
        this.tn = tn;
    }

    public String getSt() {
        return st;
    }

    public void setSt(String st) {
        this.st = st;
    }

    public String getAr() {
        return ar;
    }

    public void setAr(String ar) {
        this.ar = ar;
    }

    public String getAs() {
        return as;
    }

    public void setAs(String as) {
        this.as = as;
    }

    public String getAt() {
        return at;
    }

    public void setAt(String at) {
        this.at = at;
    }

    public String getAn() {
        return an;
    }

    public void setAn(String an) {
        this.an = an;
    }

    public String getBr() {
        return br;
    }

    public void setBr(String br) {
        this.br = br;
    }

    public String getBs() {
        return bs;
    }

    public void setBs(String bs) {
        this.bs = bs;
    }

    public String getBt() {
        return bt;
    }

    public void setBt(String bt) {
        this.bt = bt;
    }

    public String getBn() {
        return bn;
    }

    public void setBn(String bn) {
        this.bn = bn;
    }

    public String getCr() {
        return cr;
    }

    public void setCr(String cr) {
        this.cr = cr;
    }

    public String getCs() {
        return cs;
    }

    public void setCs(String cs) {
        this.cs = cs;
    }

    public String getCt() {
        return ct;
    }

    public void setCt(String ct) {
        this.ct = ct;
    }

    public String getCn() {
        return cn;
    }

    public void setCn(String cn) {
        this.cn = cn;
    }

    public String getDr() {
        return dr;
    }

    public void setDr(String dr) {
        this.dr = dr;
    }

    public String getDs() {
        return ds;
    }

    public void setDs(String ds) {
        this.ds = ds;
    }

    public String getDt() {
        return dt;
    }

    public void setDt(String dt) {
        this.dt = dt;
    }

    public String getDn() {
        return dn;
    }

    public void setDn(String dn) {
        this.dn = dn;
    }

    public String getFuseAr() {
        return fuseAr;
    }

    public void setFuseAr(String fuseAr) {
        this.fuseAr = fuseAr;
    }

    public String getFuseAs() {
        return FuseAs;
    }

    public void setFuseAs(String fuseAs) {
        FuseAs = fuseAs;
    }

    public String getFuseAt() {
        return FuseAt;
    }

    public void setFuseAt(String fuseAt) {
        FuseAt = fuseAt;
    }

    public String getFuseBr() {
        return fuseBr;
    }

    public void setFuseBr(String fuseBr) {
        this.fuseBr = fuseBr;
    }

    public String getFuseBs() {
        return FuseBs;
    }

    public void setFuseBs(String fuseBs) {
        FuseBs = fuseBs;
    }

    public String getFuseBt() {
        return FuseBt;
    }

    public void setFuseBt(String fuseBt) {
        FuseBt = fuseBt;
    }

    public String getFuseCr() {
        return fuseCr;
    }

    public void setFuseCr(String fuseCr) {
        this.fuseCr = fuseCr;
    }

    public String getFuseCs() {
        return FuseCs;
    }

    public void setFuseCs(String fuseCs) {
        FuseCs = fuseCs;
    }

    public String getFuseCt() {
        return FuseCt;
    }

    public void setFuseCt(String fuseCt) {
        FuseCt = fuseCt;
    }

    public String getFuseDr() {
        return fuseDr;
    }

    public void setFuseDr(String fuseDr) {
        this.fuseDr = fuseDr;
    }

    public String getFuseDs() {
        return FuseDs;
    }

    public void setFuseDs(String fuseDs) {
        FuseDs = fuseDs;
    }

    public String getFuseDt() {
        return FuseDt;
    }

    public void setFuseDt(String fuseDt) {
        FuseDt = fuseDt;
    }

    public String getNoSeri() {
        return noSeri;
    }

    public void setNoSeri(String noSeri) {
        this.noSeri = noSeri;
    }

    public String getTanggalPasang() {
        return tanggalPasang;
    }

    public void setTanggalPasang(String tanggalPasang) {
        this.tanggalPasang = tanggalPasang;
    }

    public String getTanggalGanti() {
        return tanggalGanti;
    }

    public void setTanggalGanti(String tanggalGanti) {
        this.tanggalGanti = tanggalGanti;
    }

    public String getWaktu() {
        return waktu;
    }

    public void setWaktu(String waktu) {
        this.waktu = waktu;
    }
}
